...
function FieldSet(props) {
	...
	return (
		<>
			<input
				className={`form__field form__field_theme-dark ${emailError && 'form__field_type_error'}`}
				type="email"
				placeholder="Email"
				name="email"
				value={email}
			...
			<input
				className={`form__field form__field_theme-dark ${pwdError && 'form__field_type_error'}`}
				type="password"
				placeholder="Пароль"
				name="pwd"
				minLength="8"
